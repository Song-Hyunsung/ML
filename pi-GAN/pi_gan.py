...
class piGAN:
    ...
    def __init__(self, dataset, device_ids=[0, 1, 2, 3]):
        ...
        self.optim_G = Adam(self.G.parameters(), betas=(0.0, 0.9), lr=5e-5)
        self.optim_D = Adam(self.D.parameters(), betas=(0.0, 0.9), lr=4e-4)
        ...
        self.val_dataloader = DataLoader(self.val_dataset, batch_size=self.batch_size_validation, shuffle=True)
        
        
        self.cur_resolution = 16

        self.pretrained_autoencoder = AutoEncoder(latent_dim=128, emb_dim=128).cuda()
        ckpt = torch.load("./od-encoder.pth")
        state = ckpt.get("model_state_dict", ckpt.get("state_dict", ckpt))
        self.pretrained_autoencoder.load_state_dict(state)
        self.pretrained_autoencoder.encoder.eval()
        for p in self.pretrained_autoencoder.encoder.parameters():
    ...
    def train_step(self):
        ...
        while i == 0 or (i < tiny_steps and self.loss_D > self.loss_G):
            ...
            real_imgs, center_imgs = next(self.dataloader_cycle)
            real_imgs   = real_imgs.to(self.device)
            center_imgs = center_imgs.to(self.device)
    
            with torch.no_grad():
                # rand_latents = torch.randn(self.batch_size_D, self.G.module.dim_latent).cuda()
                latents_D, _ = self.pretrained_autoencoder.encoder(center_imgs, return_skip=True)
                cam2world = sample_cam_poses(math.pi/6, 0.15, self.batch_size_D).cuda()
                fake_imgs, _ = self.G(latents_D, cam2world, 32, with_importance=self.iterations>25000)
            fake_imgs.detach_()
            fake_imgs_D_out = self.D(fake_imgs)        
            
            real_imgs = real_imgs.requires_grad_()
        ...
        while i == 0 or (i < tiny_steps and self.loss_G > self.loss_D * 2.):
            ...
            real_imgs   = real_imgs.to(self.device)
            center_imgs = center_imgs.to(self.device)
            center_imgs = center_imgs[:self.batch_size_G]
            with torch.no_grad():
                latents_G, _ = self.pretrained_autoencoder.encoder(center_imgs, return_skip=True)
            rand_latents = torch.randn(self.batch_size_G // 2, self.G.module.dim_latent).cuda().repeat((2, 1))
            cam2world = sample_cam_poses(math.pi/6, 0.15, self.batch_size_G).cuda()
            fake_imgs, _ = self.G(latents_G, cam2world, 32, with_importance=self.iterations>15000)
    ...
    def test_imgs(self):
        ...
        # grab one batch of (real, center) from val set
        real_imgs, center_imgs = next(iter(self.val_dataloader))
        real_imgs = real_imgs.cuda()
        center_imgs = center_imgs.cuda()

        with torch.no_grad():
            # only keep 16 samples
            center_imgs = center_imgs[:16]                # (16, 3, H, W)
            # encode to 128‑d latents
            latents, _ = self.pretrained_autoencoder.encoder(center_imgs, return_skip=True)
            # build a 4×4 camera grid
            cam2world = get_cam_poses(
                torch.linspace(-0.3, 0.3, 4),
                torch.zeros(4),
                torch.ones(4)
            ).cuda()                                    # (4,4,4)
            cam2world = cam2world.repeat((4, 1, 1))     # (16,4,4)

            # render at 128px
            fake_imgs, depth_imgs = self.G(latents, cam2world, 128, True)

        # depth → [0,1] gray + channel dim
        depth_imgs = torch.exp(-3. * depth_imgs + 2.)[:, None, :, :]

        # save exactly as before
        torchvision.utils.save_image(
            fake_imgs,
            os.path.join(test_save_dir, f'iter_{self.iterations:06d}_val.png'),
            nrow=4
        )
        torchvision.utils.save_image(
            depth_imgs,
            os.path.join(test_save_dir, f'iter_{self.iterations:06d}_val_depth.png'),
            nrow=4
        )
